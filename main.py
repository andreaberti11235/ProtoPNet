...
def main():
    ...
    # TODO SISTEMARE
    path_to_csv = os.path.join(original_dir, 'tutte_MLO_bm.csv')
    path_to_csv_augmented = os.path.join(augm_dir, 'tutte_MLO_augmented_bm.csv')
    ...
    # dataset of all images (train+test) non-normalized for push
    ...
    img_original_norm_dataset = datasets.ImageFolder(
        original_dir,
        transforms.Compose([
            transforms.Grayscale(num_output_channels=3), #TODO
            transforms.Resize(size=(img_size, img_size)),
    ...
    # dataset of all images augmented
    ...
    for random_state in range(8, 11):
        
        print(f'New seed {random_state}')
        # Separate train and test from original images
        ...
        # dict_norm = {key[0]:value for value, key in enumerate(img_original_norm_dataset.imgs)}
        dict_original = {key[0]:value for value, key in enumerate(img_original_dataset.imgs)}
        
        k = 5
        
        x = StratifiedGroupKFold(n_splits=k, shuffle=True, random_state=random_state) #reproducibility, 5-fold
        for fold, (all_train_idx, test_augm_idx) in enumerate(x.split(X,y,groups=group)):    
            
            print('Esco')
            break
            
        all_train_idx = [dict_augmented[X[index]] for index in all_train_idx]
        ...
        # dà la lista dei nomi delle immagini di push+valid (quindi non aumentate)
        ...
        name_prefixes_train_orig = []
        for index in all_train_idx:
            img_augm_prefix = f'{X[index].split(sep=str_sep)[0]}png'
            ...
            name_prefixes_train_orig.append(img_augm_prefix)
        train_original_names = list(set(name_prefixes_train_orig))
        
        # create dataframe
        tmp = [X[index] for index in all_train_idx]
        print(f'tmp:{tmp[:3]}')
        print(f'df_augmented.index:{df_augmented.index[:3]}')
        all_train_df = df_augmented.loc[tmp]
        all_train_original_df = df_original.loc[train_original_names]
        
        seed_dir = os.path.join('.', 'saved_models', base_architecture, f'rs_{random_state}')
        
        if not os.path.exists(seed_dir):
            os.makedirs(seed_dir)
            
        all_train_original_df.to_csv(os.path.join(seed_dir, f'train_names_rs_{random_state}.csv'), index=True)
        all_train_df.to_csv(os.path.join(seed_dir, f'train_aug_names_rs_{random_state}.csv'), index=True)

        # dà la lista dei nomi di test (non aumentati)
        name_prefixes_test = []
        for index in test_augm_idx:
            img_augm_prefix = f'{X[index].split(sep=str_sep)[0]}png'
            img_augm_prefix = img_augm_prefix.split(os.sep)
            tail = img_augm_prefix[-1]
            tail_splits = tail.split(sep='Mass')
            ...
            img_augm_prefix = os.path.join(*img_augm_prefix[-2:])
            img_augm_prefix = os.path.join(original_prefix, img_augm_prefix)
            name_prefixes_test.append(img_augm_prefix)
        test_names = list(set(name_prefixes_test))
        
        test_df = df_original.loc[test_names]
        
        test_df.to_csv(os.path.join(seed_dir, f'test_names_seed_{random_state}.csv'), index=False)
        
            

        for idx, config in enumerate(chosen_configurations): # TODO
            ...
            model_dir = os.path.join(seed_dir, experiment_run)
            makedir(model_dir)
            ...
            X2 = np.array(all_train_df.index) # is an array of names
            group2 = np.array(all_train_df['patient_id'])
            y2 = np.array(all_train_df['label'])
            
            y2 = np.array([0 if elem=='benign' else 1 for elem in y2]) #TODO modificare con la stringa opportuna
            
            
            y2_original = np.array(all_train_original_df['label'])
            y2_original = np.array([0 if elem=='benign' else 1 for elem in y2_original]) #TODO modificare con la stringa opportuna
            ...
            x2 = StratifiedGroupKFold(n_splits=k, shuffle=True, random_state=42) #reproducibility, 5-fold
            for fold, (train_idx, valid_augm_idx) in enumerate(x2.split(X2,y2,groups=group2)):
                ...
                train_idx = [dict_augmented[X2[index]] for index in train_idx]
                ...
                train_push_loader = torch.utils.data.DataLoader(img_original_dataset,
                ...
                test_loader = torch.utils.data.DataLoader(img_original_norm_dataset,
